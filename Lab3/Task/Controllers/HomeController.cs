...
namespace Application.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public HomeController(HospitalContext context, IMemoryCache _memoryCache)
        {
            this._memoryCache = _memoryCache;
            db = context;
        }

        private void LoadCookieObject()
        {
            // Load session by Extension method.
            string value = HttpContext.Request.Cookies["nameCookie"];
            ViewData["nameCookie"] = value;

            value = HttpContext.Request.Cookies["OptionOne"];
            ViewData["OptionCOne"] = value;

            value = HttpContext.Request.Cookies["OptionTwo"];
            ViewData["OptionCTwo"] = value;
        }
        ...
        public IActionResult Index()
        {
            ...
            Department orderFromMemory = (Department)_memoryCache.Get(path);
            LoadCookieObject();
            ViewData["orderFromMemory"] = orderFromMemory;
            return View();
        }
        ...
        {
            HttpContext.Response.Cookies.Append("OptionTwo", key.ToString());
            if (key == 1)
            {
                var selection = from r in db.Departments
        ...
        {
            HttpContext.Response.Cookies.Append("OptionOne", key.ToString());
            if (key == 1)
            {
                var selection = from r in db.Departments
